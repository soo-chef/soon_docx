...
**식사사진등첨부** / **식사사진등첨부2** 열은 `영양사총평` 오른쪽에 헤더 이름을 **정확히** 두면 됩니다. 두 번째 열이 비어 있으면 DOCX에는 첫 열 사진만 들어가고, 두 번째에 URL이 있으면 **같은 첨부 칸 안에서 첫 번째 사진 아래**에 이어서 들어갑니다.

- 셀에는 **이미지 주소**가 들어가야 합니다. **셀에 그림만 붙여 넣기**(이미지가 셀 안에만 보이는 방식)는 API에 URL이 없어 DOCX에 넣을 수 없습니다. 같은 파일을 Drive에 올렸다면 **그 파일의 공유 링크**를 셀에 넣거나, **삽입 → 링크**로 Drive 주소를 넣으세요(자동으로 `HYPERLINK` 수식이 되며 앱이 URL을 읽습니다).
  - **공개 `https://...` 이미지 URL** — 누구나 열 수 있는 주소
  - **Google Drive**에 올린 파일의 **공유 링크** (`drive.google.com/file/d/파일ID/...`) — 해당 파일을 **서비스 계정 `client_email`** 과 공유(뷰어 이상)해야 다운로드됩니다.
  - 여러 장: 한 셀에 줄바꿈·쉼표·세미콜론으로 구분
  - `=IMAGE("https://...")` 또는 `=HYPERLINK("https://...","표시글")` 수식 모두 인식합니다.
- DOCX **식사사진 등 첨부** 칸(영양사 총평 아래)에 넣어지고, **합본 PDF**도 LibreOffice 변환 시 같은 이미지가 포함됩니다.
